...
cout<<"Done\n";
 ...
 //     cout<<t;
    ...
    c1-=1;
    c2+=1;
    while(c1>=0&&total!=m&&flag){
   // cout<<"while\n";
    for(p=c1;p<=c2&&flag;p++)
    for(q=c1;q<=c2&&flag;q++)
     if(p>=0&&q>=0&&p<n&&q<n&&(a[p][q]==0||b[p][q]>0))
      {if(total==m)
    break;
     if(tot()<=(av-total))
    {//cout<<"(3:)\n"<<tot()<<":"<<total<<":\n";
...
//   cout<<"while2\n";
        c1-=1;
    c2+=1;
    for(p=c1;p<=c2&&flag;p+=2)
    for(q=c1;q<=c2&&flag;q+=2)
     { 
        if(total==m)
    break;
            if(p>=0&&q>=0&&p<n&&q<n&&a[p][q]==0){
 ...
 for(p=c1;p<=c2&&flag;p+=1)
    for(q=c1;q<=c2&&flag;q+=1)
     { 
        if(total==m)
    break;
            if(p>=0&&q>=0&&p<n&&q<n&&a[p][q]==0){
